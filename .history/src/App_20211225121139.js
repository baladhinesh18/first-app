...
function App() {
  ...
  return (
    <div className="App">
      {users.map(({ name, url }) => (
        <Starting nme={name} ur={url} />
      ))}
    </div>
  );
}

function Starting({ nme, ur }) {
  // let name = "Bala dhinesh";
  return (
    <div>
      <h1>
        <img src={ur} className="mg" alt="avatar" /> Hi {nme}!!!
      </h1>
    </div>
  );
}
