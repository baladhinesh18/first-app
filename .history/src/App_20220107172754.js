...
function App() {
  ...
  const[add,setAdd] =useState({name,url,rating,summary});
  console.log(add);
  return (
    <div className="App">
    <h1 className="lor">Add New Movie</h1>
      <div  className="btn" >
      <TextField id="outlined-basic" label="Enter a Movie Name" variant="outlined"  onChange={(event)=>setName(event.target.value)} />
      <TextField id="outlined-basic" label="Enter a Movie url" variant="outlined" style={mart}  onChange={(event)=>setUrl(event.target.value)}/>
      <TextField id="outlined-basic" label="Enter a Movie rating" variant="outlined" style={mart}   onChange={(event)=>setRating(event.target.value)}/>
      <TextField id="outlined-basic" label="Enter a Movie summary" variant="outlined" style={mart}  onChange={(event)=>setSummary(event.target.value)}/>
      <Button  variant="contained" style={mart} onClick={()=>setAdd([...users,add])}>Add Movie</Button>
      {/* <Counter/> */}
      </div>
