...
function App() {
  ...
  return (
    ...
    <h1 className="lor">Add New Movie</h1>
      ...
      <Button  variant="contained" style={mart} onClick={()=>setAdd([...users,x])}>Add Movie</Button>
      {/* <Counter/> */}
      </div>
...
function Starting({ name, url,ratings,summary }) {
const styles={color: ratings>=8.5? "green":"red"};
const[isHide,setIsHide]=useState(true);

  return (
    <div>
    <div className="ro">
     <img src={url} className="mg" alt={name} />
      <div className="col">
      <h1> {name}</h1>
      <h2 style={styles}>IMDB:{ratings}/10</h2>
      { isHide?<h3>description:<a href={summary}>{summary}</a></h3>:null}
