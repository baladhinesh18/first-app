...
function App() {
  ...
  return (
    ...
    <h1 className="lor">Add New Movie</h1>
      <div  className="btn" >
      <TextField id="outlined-basic" label="Enter a Movie Name" variant="outlined" value={List.name}  onChange={(event)=>setList(event.target.value)} />
      <TextField id="outlined-basic" label="Enter a Movie url" variant="outlined" style={mart} value={List.url}  onChange={(sumev)=>setList(sumev.target.value)}/>
      <TextField id="outlined-basic" label="Enter a Movie rating" variant="outlined" style={mart} value={List.rating}  onChange={(sumev)=>setList(sumev.target.value)}/>
      <TextField id="outlined-basic" label="Enter a Movie summary" variant="outlined" style={mart} value={List.summary}  onChange={(sumev)=>setList(sumev.target.value)}/>
      <Button  variant="contained" style={mart} onClick={()=>setList([...users,List.name,List.url,List.rating,List.summary])}>Add Movie</Button>
      {/* <Counter/> */}
      </div>
