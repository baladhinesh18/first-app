...
function App() {
  ...
  return (
    ...
    <h1 className="lor">Add New Movie</h1>
      <div  className="btn">
      <TextField id="outlined-basic" label="Enter a Movie Name" variant="outlined" value={List} onChange={(nameev)=>setList(nameev.target.value)} />
      <TextField id="outlined-basic" label="Enter a Movie url" variant="outlined" style={mart} value={List} onChange={(urlev)=>setList(urlev.target.value)} />
      <TextField id="outlined-basic" label="Enter a Movie rating" variant="outlined" style={mart} value={List} onChange={(rateev)=>setList(rateev.target.value)}/>
      <TextField id="outlined-basic" label="Enter a Movie summary" variant="outlined" style={mart} value={List} onChange={(sumev)=>setList(sumev.target.value)} />
      <Button  variant="contained" style={mart} onClick={()=>setList([...users,List])}>Add Movie</Button>
      {/* <Counter/> */}
      </div>
